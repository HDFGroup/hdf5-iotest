...
to configure the HDF5 library and output, perform write and read operations, and
report certain timings. (The number of combinations is different for sequential
and parallel runs, and depends on other choices.)
