...
a set of 2D array variables over a set of "time steps." A configuration file
(see below) is used to control parameters such as the number of steps, the count
and shape of the 2D array variables, etc. (See section [[sec:parameters]].) The test
then uses between 48 and 192 different combinations of up to 7 internal
parameter (see section [[sec:internal-parameters]]) to configure the HDF5 library
and output, perform write and read operations, and report certain timings. (The
number of combinations is different for sequential and parallel runs, and
...
- =creat-min [s],creat-max [s]= :: The fastest and slowest time spent in
  =H5Fcreate= and =H5Dcreate= in seconds
...
- =read-phase-min [s],read-phase-max [s]= :: The fastest and slowest cumulative
  read phase time in seconds. This includes the times for opening the HDF5 file
  and for creating dataset selections.
